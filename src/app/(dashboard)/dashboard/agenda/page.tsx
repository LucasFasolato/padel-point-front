...
import { ChevronLeft, ChevronRight, Lock, Unlock, Calendar, User, Clock, CheckCircle2 } from 'lucide-react';
...
export default function AgendaPage() {
  ...
  const [agenda, setAgenda] = useState<{ courts: string[], items: AgendaItem[] }>({ courts: [], items: [] });
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 bg-white px-6 py-4 shadow-sm">
        <div>
          ...
          <p className="text-sm text-slate-500">{club ? club.nombre : 'Cargando club...'}</p>
        </div>
        ...
        <div className="flex items-center gap-4 rounded-xl bg-slate-100 p-1">
          ...
          <div className="flex items-center gap-2 px-2">
            ...
            <span className="font-semibold text-slate-700 capitalize w-32 text-center">
      ...
      <div className="flex-1 overflow-auto bg-slate-50 p-6">
        {loading ? (
           <div className="flex h-full items-center justify-center text-slate-400">Cargando agenda...</div>
          ...
          <div className="min-w-[800px] rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
            {/* Grid Header (Courts) */}
            <div className="flex border-b border-slate-200 bg-slate-50">
              <div className="w-20 flex-shrink-0 border-r border-slate-200 p-4 font-bold text-slate-400 text-center">Hora</div>
              {agenda.courts.map((courtName, idx) => (
                <div key={idx} className="flex-1 border-r border-slate-200 p-4 text-center font-bold text-slate-700 last:border-0">
                  {courtName}
                </div>
              ))}
            </div>
              ...
              {hours.map((hour) => {
                ...
                return (
                  <div key={hour} className="flex h-20">
                    <div className="flex w-20 flex-shrink-0 items-center justify-center border-r border-slate-100 bg-slate-50 text-sm font-medium text-slate-500">
                      {timeStr}
                    </div>
                    {agenda.courts.map((courtName, idx) => {
                      // Find the item for this slot
                      const item = agenda.items.find(i => i.courtNombre === courtName && i.start.startsWith(timeStr)); // Simplified matching
                      ...
                      const displayItem = item || { status: 'free', start: timeStr, end: `${hour+1}:00`, courtNombre: courtName, courtId: 'mock' } as AgendaItem;
                        ...
                        <div key={idx} className="flex-1 border-r border-slate-100 p-1 last:border-0">
                          <button
                            ...
                            className={cn(
                              ...
                              displayItem.status === 'free' && "border-slate-100 bg-white hover:border-blue-300",
                              displayItem.status === 'occupied' && "border-green-200 bg-green-50 text-green-800",
                              displayItem.status === 'blocked' && "border-red-200 bg-red-50 text-red-800"
                            )}
                          >
                            {displayItem.status === 'occupied' ? (
                                <>
                                    <div className="flex items-center gap-1 text-xs font-bold"><User size={12}/> {displayItem.detail?.clientName || 'Cliente'}</div>
      ...
      <Dialog.Root open={isModalOpen} onOpenChange={setIsModalOpen}>
        <Dialog.Portal>
            ...
            <Dialog.Content className="fixed left-1/2 top-1/2 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-2xl bg-white p-6 shadow-2xl z-50 animate-slide-up">
                <div className="mb-4 flex items-center justify-between">
                    <h3 className="text-lg font-bold text-slate-900">Gestionar Horario</h3>
                    <Dialog.Close className="text-slate-400 hover:text-slate-600"><Lock size={18}/></Dialog.Close>
                </div>

                {selectedItem && (
                    <div className="space-y-4">
                        <div className="flex items-center justify-between rounded-lg bg-slate-50 p-3 text-sm text-slate-600">
                            <span className="flex items-center gap-2"><Clock size={16}/> {selectedItem.start} - {selectedItem.end}</span>
                            <span className="font-bold">{selectedItem.courtNombre}</span>
                        </div>

                        {selectedItem.status === 'free' && (
                            <div className="space-y-3">
                                <p className="text-sm text-slate-500">Este horario está libre. ¿Qué deseas hacer?</p>
                                <button 
                                ...
                                >
                                    {processing ? 'Procesando...' : <><Lock size={16}/> Bloquear Horario</>}
                                </button>
                                <button className="w-full rounded-xl border border-slate-200 py-3 font-medium text-slate-600 hover:bg-slate-50">
                        ...
                        {selectedItem.status === 'blocked' && (
                             <div className="space-y-3">
                                ...
                                <button 
                                    ...
                                    disabled={processing}
                                    className="flex w-full items-center justify-center gap-2 rounded-xl border border-red-200 bg-white py-3 font-bold text-red-600 hover:bg-red-50"
                                >
                        ...
                        {selectedItem.status === 'occupied' && (
                            <div className="space-y-3">
                                <div className="rounded-xl border border-green-100 bg-green-50 p-4">
                                    ...
                                    <p className="text-sm text-green-800">Cliente: {selectedItem.detail?.clientName || 'Usuario Web'}</p>
                                    <p className="text-sm text-green-800">Pago: {selectedItem.detail?.price ? formatCurrency(selectedItem.detail.price) : 'Pendiente'}</p>
                                </div>
                                <button className="w-full text-sm text-slate-400 hover:text-red-500 hover:underline">
                                    Cancelar Reserva
                                </button>
                            </div>
                        )}
