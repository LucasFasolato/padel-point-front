...
function getDeltaUi(delta: number | undefined, isNew: boolean): {
  ...
  if (delta < 0) {
    return { symbol: '▲', value: `+${Math.abs(delta)}`, className: 'text-[#0E7C66]' };
  }
...
function diffColor(value?: number): string {
  ...
  return value > 0 ? 'text-[#0E7C66]' : 'text-rose-500';
}
...
export function StandingsTable({
  ...
  if (isLoading) {
    return (
      <div className={cn('space-y-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm', className)}>
  ...
  if (standings.length === 0) {
    return (
      <div className={cn('rounded-2xl border border-dashed border-slate-200 bg-[#F7F8FA] py-10 text-center', className)}>
  ...
  return (
    <div className={cn('space-y-2', className)}>
      {topMovers && (
        <div className="flex flex-wrap gap-3 rounded-2xl border border-slate-100 bg-white px-3 py-2 text-xs shadow-sm">
          {topMovers.up.length > 0 && (
            <div className="flex items-center gap-1.5">
              <span className="font-medium text-[#0E7C66]">▲</span>
              {topMovers.up.map((m) => (
                <span key={m.displayName} className="inline-flex items-center gap-0.5 rounded-full bg-[#0E7C66]/10 px-2 py-0.5 text-[#0E7C66]">
      ...
      <table className="w-full text-sm">
        <thead className="bg-slate-50">
          <tr>
            ...
            <th className="w-9 px-2 py-2.5 text-center text-xs font-semibold text-[#0E7C66]" title="Victorias">G</th>
        ...
        <tbody className="divide-y divide-slate-100">
          {standings.map((entry) => {
            ...
            return (
              <tr
                key={entry.userId}
                className={cn('transition-colors', isMe && 'bg-[#0E7C66]/5 font-medium')}
              >
                ...
                <td className="max-w-[130px] px-2 py-3 text-slate-900">
                  <div className="flex min-w-0 items-center gap-1.5">
                    ...
                    {isMe && (
                      <span className="shrink-0 rounded-full bg-[#0E7C66]/10 px-1.5 py-0.5 text-[10px] font-semibold text-[#0E7C66]">
                        Vos
                      </span>
                    )}
                  </div>
                ...
                <td className="px-2 py-3 text-center font-semibold text-[#0E7C66]">{entry.wins}</td>
